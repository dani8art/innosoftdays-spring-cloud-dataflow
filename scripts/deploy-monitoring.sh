...
EOF
    ...
    helm upgrade --wait -n "${grafana_namespace}" --install "${grafana_release_name}" \
        ...
        --set plugins=grafana-image-renderer \
...
echo "A release of bitnami/grafana, ${grafana_release_name}, is running on ${grafana_namespace} namespace"

echo "PROMETHEUS RESOURCES"
kubectl get -n "${prometheus_namespace}" ingress,pod,svc,job,cronjob,deployment,rs,pv,pvc,secret,ep -o wide -l "app.kubernetes.io/instance=${prometheus_release_name}"

echo ""

echo "GRAFANA RESOURCES"
kubectl get -n "${grafana_namespace}" ingress,pod,svc,job,cronjob,deployment,rs,pv,pvc,secret,ep -o wide -l "app.kubernetes.io/instance=${grafana_release_name}"
